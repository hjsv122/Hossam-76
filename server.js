...
app.post('/withdraw', async (req, res) => {
    try {
        ...
        if (!tronWeb.isAddress(toAddress)) {
            return res.status(400).json({ success: false, message: 'عنوان محفظة غير صالح' });
        }

        if (amount < 250) {
            return res.status(400).json({ success: false, message: 'الحد الأدنى للسحب هو 250 USDT' });
        }
        ...
        res.json({ success: true, message: `تم إرسال ${finalAmount} USDT بنجاح`, tx });
    } catch (error) {
        console.error(error);
        res.status(500).json({ success: false, message: 'فشل عملية السحب' });
    }
});
